...
import { localizeAchievements } from "../app/features/achievements/achievement-copy";
import { LanguageSwitcher } from "../app/i18n/language-switcher";
import { useI18n } from "../app/i18n/provider";
import { LegalLinks } from "../app/legal/legal-links";
import { titleSearchText } from "../app/i18n/title-copy";
import { browserDateLocale } from "../app/i18n/locale";
...
function globalHitsFor(rawQuery: string): GlobalHit[] {
  ...
  for (const item of ITEMS) {
    ...
    const titleMatch = normalize(titleSearchText(item)).includes(query);
    if (titleMatch)
      hits.push({
        key: `title-${item.id}`,
        item,
  ...
  const searchResults = useMemo(
    () =>
      query.trim()
        ? ITEMS.filter(
            (item) =>
              normalize(titleSearchText(item)).includes(normalize(query)) &&
              (!spoilerSafe ||
                watched.has(item.id) ||
                !ITEMS.some(
                  (entry) =>
...
const formatDate = (iso: string) =>
  dateAtNoon(iso).toLocaleDateString(browserDateLocale(), {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
                    ...
                    return (
                      <section key={date} className={`marathon-day${isToday ? " today" : ""}`}>
                        <header>
                          ...
                          <span>
                            <strong>
                              {dateAtNoon(date).toLocaleDateString(browserDateLocale(), {
                                weekday: "long",
                                day: "numeric",
                                month: "long",
                              })}
      ...
      <section className="achievement-detail" data-tier={achievement.tier.toLowerCase()}>
        ...
        <div className="achievement-detail-progress">
          ...
          {achievement.unlockedAt && (
            <p>
              Obtenido el{" "}
              {new Intl.DateTimeFormat(browserDateLocale(), { dateStyle: "long" }).format(
                  ...
                  return (
                    <article key={marathon.id} className="marathon-library-card">
                      ...
                      <div className="marathon-library-copy">
                        <small>
                          {marathon.author} ·{" "}
                          {new Date(marathon.createdAt).toLocaleDateString(browserDateLocale())}
                        </small>
                        <h3>{marathon.name}</h3>
                        <p>

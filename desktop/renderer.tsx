...
export function App() {
  ...
  const legacyAchievements = useMemo(() => {
    ...
    const completedSagas=sagas.filter((saga)=>releasedItems.filter((item)=>item.saga===saga).every((item)=>watched.has(item.id))).length;
    const ironJourney=["iron-man","iron-man-2","avengers","iron-man-3","age-ultron","civil-war","infinity-war","endgame"].filter((id)=>ITEM_BY_ID.has(id));
    const spiderJourney=["spiderman-raimi-1","spiderman-raimi-2","spiderman-raimi-3","no-way-home"].filter((id)=>ITEM_BY_ID.has(id));
    const journeyProgress=(ids:string[])=>ids.filter((id)=>watched.has(id)).length/Math.max(1,ids.length);
    ...
    return [
      ...
      {id:"phase",title:"Fase completada",description:"Completa todas las historias de una fase del UCM",icon:"trophy",unlocked:completedPhases>=1,progress:Math.min(1,completedPhases)},
      {id:"saga",title:"Guardián de sagas",description:"Completa una saga audiovisual",icon:"route",unlocked:completedSagas>=1,progress:Math.min(1,completedSagas)},
      {id:"iron-journey",title:"Legado de hierro",description:"Completa el viaje esencial de Iron Man",icon:"spark",unlocked:journeyProgress(ironJourney)>=1,progress:journeyProgress(ironJourney)},
...
function AchievementsView({achievements,watched,onOpenRoute}:{achievements:Achievement[];watched:Set<string>;onOpenRoute:(achievement:Achievement)=>void}) {
  ...
  return <section className="dashboard-workspace achievements-workspace">
    ...
    <div className="dashboard-scroll achievements-scroll">
      {next&&<section className="next-achievement" data-tier={next.tier.toLowerCase()}><span className="next-achievement-hero" style={{backgroundImage:`url(${achievementVisual(next).hero})`}} aria-hidden="true"/><span className="next-achievement-badge" style={{backgroundImage:`url(${achievementVisual(next).thumb})`}} aria-hidden="true"/><div><small>Siguiente insignia · {next.tier}</small><h2>{next.title}</h2><p>{next.description}</p><i><b style={{width:`${next.progress*100}%`}}/></i></div><strong>{next.current}/{next.goal}</strong><button onClick={()=>setSelectedAchievement(next)}>Ver requisitos</button></section>}
      {near.length>0&&<section className="achievement-near"><div><span className="dash-eyebrow">Casi completados</span><h2>Lo tienes cerca</h2></div><div>{near.map((entry)=><button key={entry.id} onClick={()=>setSelectedAchievement(entry)} style={{backgroundImage:`linear-gradient(90deg,rgba(11,14,19,.12),rgba(11,14,19,.98) 47%),url(${achievementVisual(entry).thumb})`}}><span>{Math.round(entry.progress*100)}%</span><strong>{entry.title}</strong></button>)}</div></section>}
      <section className="achievement-browser">
        ...
        <div className="achievement-group-rail">{groups.map((entry)=>{const sample=achievements.find((achievement)=>entry==="Todos"||achievementGroup(achievement.id)===entry);return <button key={entry} className={group===entry?"active":""} onClick={()=>setGroup(entry)} style={{backgroundImage:`linear-gradient(90deg,rgba(12,15,20,.08),rgba(12,15,20,.96) 48%),url(${sample?achievementVisual(sample).thumb:"./artwork/cosmic-hero-v1.webp"})`}}><span>{entry}</span><small>{entry==="Todos"?achievements.length:achievements.filter((achievement)=>achievementGroup(achievement.id)===entry).length}</small></button>})}</div>
      </section>
      ...
      {visible.length?<div className="achievement-catalog">{visible.map((achievement)=>{const visual=achievementVisual(achievement);return <button type="button" key={achievement.id} data-tier={achievement.tier.toLowerCase()} className={achievement.unlocked?"unlocked":""} onClick={()=>setSelectedAchievement(achievement)}><span className="achievement-thumb" style={{backgroundImage:`url(${visual.thumb})`}} aria-hidden="true"/><div><small>{achievementGroup(achievement.id)} · {achievement.tier}</small><strong>{achievement.title}</strong><p>{achievement.description}</p><i><b style={{width:`${achievement.progress*100}%`}}/></i><em>{achievement.unlocked?'Desbloqueado':`${achievement.current}/${achievement.goal} · ${Math.round(achievement.progress*100)}%`}</em></div>{achievement.unlocked?<Icon name="check"/>:<Icon name="chevron"/>}</button>})}</div>:<div className="list-empty"><Icon name="trophy" size={36}/><h2>No hay logros en este filtro</h2><p>Prueba otra categoría, búsqueda o vuelve a mostrar todos.</p></div>}
    </div>
...
function AchievementDetail({achievement,watched,spoilerSafe=false,onClose,onOpenRoute}:{achievement:Achievement;watched:Set<string>;spoilerSafe?:boolean;onClose:()=>void;onOpenRoute:()=>void}){
  ...
  return <div className="achievement-detail-layer" role="dialog" aria-modal="true" aria-label={`Logro ${achievement.title}`} onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}>
    <section className="achievement-detail" data-tier={achievement.tier.toLowerCase()}>
      <div className="achievement-detail-art" style={{backgroundImage:`linear-gradient(90deg,rgba(8,10,14,.18),rgba(8,10,14,.78)),url(${visual.hero})`}}><span className="achievement-detail-badge" style={{backgroundImage:`url(${visual.badge})`}} aria-hidden="true"/><span className="achievement-art-credit">Arte original SFW de Nexus</span><button onClick={onClose} aria-label="Cerrar"><Icon name="close"/></button></div>
      <header><span className="achievement-header-badge" style={{backgroundImage:`url(${visual.thumb})`}} aria-hidden="true"/><div><small>{achievementGroup(achievement.id)} · {achievement.tier}</small><h2>{achievement.title}</h2><p>{achievement.description}</p></div></header>

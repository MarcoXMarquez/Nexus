...
function createWindow() {
  ...
  if (smokeScreenshot) {
    window.webContents.once("did-finish-load", async () => {
      ...
      process.exitCode = checks.stations === 112 && checks.tracks === 9 && checks.hasMap && checks.hasDetails && !checks.initialHasDetails && checks.episodeButtons === 6 && checks.hasSearch ? 0 : 1;
      app.quit();
      } catch (error) {

...
export function decodeMarathonCode(
  code: string,
  validTitleIds?: ReadonlySet<string>,
): PortableMarathon {
  const clean = code.trim().replace(/\s+/g, "");
  const [prefix, encoded, expectedChecksum, ...extra] = clean.split(".");
  if (prefix !== "NXS1" || !encoded || !expectedChecksum || extra.length)
    throw new Error("El código no tiene un formato Nexus válido.");
  ...
  if (checksum(bytes) !== expectedChecksum.toUpperCase())
    throw new Error("El código está incompleto o fue modificado.");
  let payload: CompactPayload;
  try {
    payload = JSON.parse(new TextDecoder().decode(bytes)) as CompactPayload;
  } catch {
    throw new Error("No se pudo leer el contenido del código.");
  }
  if (
    payload.v !== 1 ||
    ...
    throw new Error("Esta versión del código no es compatible.");
  ...
  const tasks = payload.i.map((entry) => {
    if (!Array.isArray(entry) || typeof entry[0] !== "string" || !entry[0])
      throw new Error("El código contiene un título inválido.");
    if (validTitleIds && !validTitleIds.has(entry[0]))
      throw new Error(`El título ${entry[0]} no existe en este catálogo.`);
    const episode = entry[1];
    if (episode !== undefined && (!Number.isInteger(episode) || episode < 1 || episode > 1000))
      throw new Error("El código contiene un capítulo inválido.");

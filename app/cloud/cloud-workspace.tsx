...
function localMarathons(): LocalMarathon[] {
  try {
    return JSON.parse(localStorage.getItem(NEXUS_KEYS.customMarathons) || "[]");
  } catch {
    return [];
  }
}
...
function unlockedAchievementIds() {
  try {
    return JSON.parse(localStorage.getItem(NEXUS_KEYS.achievements) || "[]") as string[];
  } catch {
    return [];
  }
}
...
export function CloudWorkspace(props: Props) {
  ...
  const [tab, setTab] = useState<CloudTab>(() =>
    typeof window !== "undefined" && localStorage.getItem(PENDING_INVITE_KEY)
      ? "marathons"
      : "account",
  );
  ...
  const [message, setMessage] = useState(
    cloudConfigured
      ? "Modo invitado · guardado en este dispositivo"
      : "Configura Supabase para activar la nube",
  );
  ...
  const activeCloudProfile = useMemo(
    () =>
      profiles.find((profile) => profile.local_key === props.activeProfileId) ||
      profiles[0] ||
      null,
  ...
  const refreshCloudData = useCallback(
    async (activeSession: Session, knownProfiles?: CloudProfile[]) => {
      const nextProfiles =
        knownProfiles || (await upsertLocalProfiles(activeSession, cloudLocalProfiles));
      setProfiles(nextProfiles);
      for (const profile of nextProfiles) {
        if (!cloudLocalProfiles.some((local) => local.id === profile.local_key)) {
          addCloudLocalProfile({
            id: profile.local_key,
            name: profile.name,
            avatar: profile.avatar,
            color: profile.color,
            child: profile.child_mode,
          });
        }
      }
      ...
      const selected =
        nextProfiles.find((profile) => profile.local_key === cloudActiveProfileId) ||
        nextProfiles[0];
  ...
  const runSync = useCallback(
    async (preference: "merge" | "local" | "cloud" = "merge", knownProfiles?: CloudProfile[]) => {
      if (!session || !navigator.onLine) {
        setStatus(session ? "offline" : cloudConfigured ? "guest" : "unconfigured");
        setMessage(
          session
            ? "Sin conexión · los cambios quedan guardados localmente"
            : "Modo invitado · guardado en este dispositivo",
        );
        return;
      }
      ...
      try {
        const nextProfiles = knownProfiles?.length
          ? knownProfiles
          : profiles.length
            ? profiles
            : await refreshCloudData(session);
        const syncedAt = await syncProfile(
          session,
          nextProfiles,
        ...
        const selected =
          nextProfiles.find((profile) => profile.local_key === props.activeProfileId) ||
          nextProfiles[0];
        if (selected) {
        ...
        if (/desvinculado/i.test(errorMessage(error))) {
          await signOut("local");
          setSession(null);
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const { data: listener } = client.auth.onAuthStateChange((_event, nextSession) => {
      ...
      if (!nextSession) {
        ...
        setMarathons([]);
        setStatus("guest");
        setMessage("Modo invitado · guardado en este dispositivo");
      }
    });
    return () => {
      mounted = false;
  ...
  useEffect(() => {
    ...
    const offline = () => {
      setStatus("offline");
      setMessage("Sin conexión · los cambios quedan guardados localmente");
    };
  ...
  useEffect(() => {
    ...
    return () => {
      window.clearTimeout(timer);
      window.removeEventListener("nexus:local-change", schedule);
    };
  }, [runSync, session]);
  ...
  useEffect(() => {
    window.nexusCloud = {
      ...
      shareMarathon: async (marathon) => {
        if (!session || !activeCloudProfile)
          return { ok: false, error: "Inicia sesión para compartir mediante enlace." };
        try {
      ...
      importInvitation: async (token) => {
        if (!activeCloudProfile)
          return { ok: false, error: "Inicia sesión y selecciona un perfil." };
        try {
          await acceptInvitation(token, activeCloudProfile.id);
          setMarathons(await listMarathons(activeCloudProfile.id));
          return { ok: true };
        } catch (error) {
          return { ok: false, error: errorMessage(error) };
  ...
  const statusButton = (
    <button
      className={`cloud-status cloud-${status}`}
      onClick={() => window.dispatchEvent(new CustomEvent("nexus:open-cloud"))}
      title={message}
    >
      <i />
      <span>
        <strong>{session ? session.user.email?.split("@")[0] : "Invitado"}</strong>
        <small>
          {status === "synced"
            ? "Sincronizado"
            : status === "syncing"
      ...
      {props.open && (
        ...
        >
          <aside
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Cuenta y Nexus Cloud"
          >
            <header className="cloud-panel-header">
              <div>
                <span>Nexus Cloud</span>
                <h2>{session ? "Tu cuenta" : "Guarda tu multiverso"}</h2>
                <p>{message}</p>
              </div>
                ...
                <div className="cloud-panel-scroll">
                  {tab === "account" && (
                    <AccountTab
                      ...
                      onSignOut={async () => {
                        await signOut();
                        localStorage.removeItem("nexus-guest-entry-v1");
                        window.location.reload();
                      }}
                      onSignOutAll={async () => {
                  ...
                  {tab === "marathons" && (
                    <MarathonsTab
                      local={localMarathons()}
                      cloud={marathons}
                      inviteToken={inviteToken}
                      onInviteToken={setInviteToken}
                      onJoin={joinMarathon}
                      onUpload={uploadLocalMarathon}
                      onInvite={inviteTo}
                      onCopy={importCloudMarathon}
                      busy={busy}
                    />
                  )}
                  {tab === "achievements" && (
                    <AchievementsTab ids={unlockedAchievementIds()} profile={activeCloudProfile} />
                  )}
                  {tab === "devices" && (
                    <DevicesTab
                      devices={devices}
                      ...
                      onRevoke={async (id) => {
                        ...
                        if (session) setDevices(await listDevices(session));
                      }}
                    />
                  )}
                  {tab === "privacy" && (
...
function CloudSetupNotice() {
  return (
    <div className="cloud-setup">
      <b>Modo invitado listo</b>
      <p>
        Nexus ya funciona localmente. Para activar cuentas y sincronización crea tu proyecto
        gratuito de Supabase y copia las dos variables públicas indicadas en{" "}
        <code>.env.example</code>.
      </p>
      <ol>
        <li>Crea el proyecto.</li>
        <li>Ejecuta la migración incluida.</li>
        <li>Añade las variables.</li>
        <li>Reinicia Nexus.</li>
      </ol>
      <small>No hace falta desplegar la web para probarlo localmente.</small>
    </div>
  );
}
...
function AuthView(props: {
  ...
  onMode: (mode: "signin" | "signup" | "recover") => void;
  email: string;
  password: string;
  displayName: string;
  ...
  return (
    <div className="cloud-auth">
      <div className="cloud-auth-benefits">
        <b>Continúa en cualquier dispositivo</b>
        <ul>
          <li>Sincroniza películas y capítulos.</li>
          <li>Comparte maratones por invitación.</li>
          <li>Mantén perfiles y logros separados.</li>
          <li>Usa Nexus sin conexión.</li>
        </ul>
      </div>
      <form onSubmit={props.onSubmit}>
        ...
        <button className="cloud-primary" disabled={props.busy}>
          {props.busy
            ? "Procesando…"
            : props.mode === "signin"
              ? "Iniciar sesión"
        ...
        <button
          ...
          onClick={() => props.onMode(props.mode === "recover" ? "signin" : "recover")}
        >
          {props.mode === "recover" ? "Volver al inicio" : "Olvidé mi contraseña"}
        </button>
        {props.message && <p className="cloud-form-message">{props.message}</p>}
      </form>
    </div>
  );
}
  ...
  return (
    <section className="cloud-section">
      <div className="account-identity">
        <div>{(props.session.user.email || "N").slice(0, 1).toUpperCase()}</div>
        <span>
          <strong>{props.session.user.user_metadata.display_name || "Cuenta Nexus"}</strong>
          <small>{props.session.user.email}</small>
        </span>
      </div>
      <div className="sync-card">
        <i className={`sync-dot ${props.status}`} />
        <span>
          <strong>
            {props.status === "synced"
              ? "Todo sincronizado"
          ...
          <small>
            {props.status === "offline"
              ? "Se sincronizará automáticamente al volver"
              : `Última sincronización: ${readableDate(props.lastSync)}`}
          </small>
        </span>
      </div>
      <p className="section-copy">
        Nexus guarda automáticamente películas, capítulos, maratones, preferencias y logros. No
        necesitas pulsar ningún botón.
      </p>
      <button className="cloud-secondary" onClick={props.onSignOut}>
        Cerrar sesión en este dispositivo
      </button>
      <button className="cloud-secondary" onClick={props.onSignOutAll}>
...
function MarathonsTab(props: {
  local: LocalMarathon[];
  cloud: CloudMarathon[];
  inviteToken: string;
  onInviteToken: (v: string) => void;
  onJoin: () => void;
  onUpload: (m: LocalMarathon) => void;
  onInvite: (m: CloudMarathon) => void;
  onCopy: (m: CloudMarathon) => void;
  busy: boolean;
}) {
  return (
    <section className="cloud-section">
      <div className="join-marathon">
        <h3>Unirse con invitación</h3>
        <div>
          <input
            value={props.inviteToken}
            onChange={(event) => props.onInviteToken(event.target.value)}
            placeholder="Pega el código o token"
          />
          <button onClick={props.onJoin} disabled={props.busy || !props.inviteToken.trim()}>
            Unirme
          </button>
        </div>
      </div>
      ...
      {props.local.length ? (
        <div className="cloud-marathon-list">
          {props.local.map((marathon) => (
            <article key={marathon.id}>
              <div className="mini-collage">
                {marathon.coverIds.slice(0, 4).map((id) => (
                  <span key={id} style={{ backgroundImage: `url(${posterForId(id) || ""})` }} />
                ))}
              </div>
              <span>
                <strong>{marathon.name}</strong>
                <small>{marathon.tasks.length} elementos · todavía local</small>
              </span>
              <button onClick={() => props.onUpload(marathon)}>Subir y compartir</button>
            </article>
          ))}
        </div>
      ) : (
        <p className="cloud-empty">Crea un maratón desde el planificador para compartirlo.</p>
      )}
      <h3>En Nexus Cloud</h3>
      {props.cloud.length ? (
        <div className="cloud-marathon-list">
          {props.cloud.map((marathon) => (
            <article key={marathon.id}>
              <div className="mini-collage">
                {marathon.cover_ids.slice(0, 4).map((id) => (
                  <span key={id} style={{ backgroundImage: `url(${posterForId(id) || ""})` }} />
                ))}
              </div>
              <span>
                <strong>{marathon.name}</strong>
                <small>
                  {marathon.visibility === "public"
                    ? "Público"
                    : marathon.visibility === "invite"
                      ? "Con invitación"
                      : "Privado"}{" "}
                  · {marathon.marathon_items?.length || 0} elementos
                </small>
              </span>
              <div className="cloud-marathon-actions">
                <button onClick={() => props.onCopy(marathon)}>Importar copia</button>
                <button onClick={() => props.onInvite(marathon)}>Invitar</button>
              </div>
            </article>
          ))}
        </div>
      ) : (
        <p className="cloud-empty">Todavía no tienes maratones en la nube.</p>
      )}
    </section>
  );
}
...
function AchievementsTab({ ids, profile }: { ids: string[]; profile: CloudProfile | null }) {
  return (
    <section className="cloud-section">
      <div className="achievement-cloud-summary">
        ...
        <span>
          ...
          <small>
            {profile?.visibility === "public"
              ...
              : "Privados hasta que cambies la visibilidad del perfil"}
          </small>
        </span>
      </div>
      {ids.length ? (
        <div className="achievement-cloud-list">
          {ids.map((id) => (
            <span key={id}>★ {id.replace(/-/g, " ")}</span>
          ))}
        </div>
      ) : (
        <p className="cloud-empty">
          Tus primeros logros aparecerán aquí cuando avances por el multiverso.
        </p>
      )}
    </section>
  );
...
function DevicesTab({
  ...
  return (
    <section className="cloud-section">
      <p className="section-copy">
        Aquí puedes reconocer y desvincular navegadores o computadoras. El progreso local del
        dispositivo no se borra al desvincularlo.
      </p>
      <div className="device-list">
        {devices.map((device) => (
          <article key={device.id}>
            <i className={device.revoked_at ? "revoked" : ""} />
            <span>
              <strong>
                {device.name}
                {device.id === currentId ? " · Este dispositivo" : ""}
              </strong>
              <small>
                {device.platform} · Último uso {readableDate(device.last_seen_at)}
              </small>
            </span>
            {device.id !== currentId && !device.revoked_at && (
              <button onClick={() => onRevoke(device.id)}>Desvincular</button>
            )}
          </article>
        ))}
      </div>
  ...
  return (
    <section className="cloud-section">
      <div className="danger-zone">
        <h3>Eliminar la cuenta definitivamente</h3>
        <p>
          Esta acción elimina la cuenta, sus perfiles, el progreso sincronizado y los maratones
          propios.
        </p>
        <label>
          Escribe <b>ELIMINAR</b>
          <input
            value={props.confirmation}
            onChange={(event) => props.onConfirmation(event.target.value)}
          />
        </label>
        <button disabled={props.busy || props.confirmation !== "ELIMINAR"} onClick={props.onDelete}>
          Eliminar mi cuenta cloud
        </button>
      </div>
    </section>
  );

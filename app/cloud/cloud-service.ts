...
export async function upsertLocalProfiles(
  session: Session,
  ...
  if (rows.length) {
    const { error } = await client
      ...
      .upsert(rows, { onConflict: "owner_id,local_key" });
...
export async function updateCloudProfile(
  profileId: string,
  values: Partial<Pick<CloudProfile, "name" | "avatar" | "color" | "child_mode" | "visibility">>,
) {
  const client = getSupabase();
  if (!client) throw new Error("Supabase todavía no está configurado.");
  ...
  if (values.visibility) {
    ...
    const achievements = await client
      ...
      .update({ visibility: achievementVisibility })
...
export async function registerDevice(session: Session): Promise<void> {
  ...
  if (existing.data?.revoked_at)
    throw new Error(
      "Este dispositivo fue desvinculado. Inicia sesión nuevamente si deseas volver a usarlo.",
    );
...
export async function syncProfile(
  ...
  prefer: "merge" | "local" | "cloud" = "merge",
) {
  const client = getSupabase();
  if (!client) throw new Error("Supabase todavía no está configurado.");
  ...
  const remoteChanged = Boolean(
    remote &&
    (!previousSync || new Date(remote.updatedAt).getTime() > new Date(previousSync).getTime()),
  );
...
function mergeSnapshots(older: NexusSnapshot, newer: NexusSnapshot): NexusSnapshot {
  ...
  values[NEXUS_KEYS.activity] = Object.fromEntries(
    [...new Set([...Object.keys(olderActivity), ...Object.keys(newerActivity)])].map((id) => [
      id,
      Math.max(olderActivity[id] || 0, newerActivity[id] || 0),
    ]),
  );
  const olderWatched = new Set(
    Array.isArray(older.values[NEXUS_KEYS.watched])
      ? (older.values[NEXUS_KEYS.watched] as string[])
      : [],
  );
  const newerWatched = new Set(
    Array.isArray(newer.values[NEXUS_KEYS.watched])
      ? (newer.values[NEXUS_KEYS.watched] as string[])
      : [],
  );
  const allProgressIds = new Set([
    ...olderWatched,
    ...newerWatched,
    ...Object.keys(olderActivity),
    ...Object.keys(newerActivity),
  ]);
  values[NEXUS_KEYS.watched] = [...allProgressIds].filter((id) =>
    (newerActivity[id] || 0) >= (olderActivity[id] || 0)
      ? newerWatched.has(id)
      : olderWatched.has(id),
  );
  ...
  for (const id of new Set([...Object.keys(olderDates), ...Object.keys(newerDates)])) {
    const value =
      (newerActivity[id] || 0) >= (olderActivity[id] || 0) ? newerDates[id] : olderDates[id];
...
export async function listMarathons(profileId: string): Promise<CloudMarathon[]> {
  ...
  query = joinedIds.length
    ? query.or(`owner_profile_id.eq.${profileId},id.in.(${joinedIds.join(",")})`)
    : query.eq("owner_profile_id", profileId);
  const { data, error } = await query.order("updated_at", { ascending: false });
  if (error) throw error;
  return (data || []) as CloudMarathon[];
}

export async function uploadMarathon(
  profileId: string,
  marathon: LocalMarathon,
  visibility: "private" | "invite" | "public" = "invite",
) {
  const client = getSupabase();
  if (!client) throw new Error("Supabase todavía no está configurado.");
  const shareSlug =
  ...
  const { data, error } = await client
    ...
    .upsert(
      ...
      { onConflict: "owner_profile_id,source_local_id" },
    )
    .select("*")
    .single();
  if (error) throw error;
  ...
  const items = marathon.tasks.map((task, index) => ({
    marathon_id: data.id,
    position: (index + 1) * 1000,
    title_id: task.itemId,
  ...
  return {
    marathon: data as CloudMarathon,
    url: shareSlug ? `${publicAppUrl()}/marathon/${shareSlug}` : undefined,
  };
}

export async function createInvitation(
  marathonId: string,
  role: "editor" | "participant" | "viewer" = "participant",
) {
  const client = getSupabase();
  if (!client) throw new Error("Supabase todavía no está configurado.");
...
export async function acceptInvitation(token: string, profileId: string) {
  const client = getSupabase();
  if (!client) throw new Error("Supabase todavía no está configurado.");
  const { data, error } = await client.rpc("accept_marathon_invitation", {
    invitation_token: token,
    joining_profile_id: profileId,
  });
  if (error) throw error;
  return data;
}
...
export async function syncAchievements(
  ...
  records: Record<
    string,
    { version?: number; unlockedAt?: string; progressSnapshot?: unknown }
  > = {},
) {
  const client = getSupabase();
  ...
  const rows = achievementIds.map((achievementId) => ({
    ...
    unlocked_at: records[achievementId]?.unlockedAt || new Date().toISOString(),
    progress_snapshot: {
      version: records[achievementId]?.version || 1,
      ...(records[achievementId]?.progressSnapshot &&
      typeof records[achievementId].progressSnapshot === "object"
        ? (records[achievementId].progressSnapshot as object)
        : {}),
    },
    visibility: "private",
  }));
  const { error } = await client
    .from("user_achievements")
    .upsert(rows, { onConflict: "profile_id,achievement_id" });
  if (error) throw error;
}
...
export async function syncStructuredProfile(profileId: string) {
  ...
  const titleIds = new Set<string>([
    ...
    ...Object.keys(ratings),
    ...Object.keys(notes),
    ...Object.keys(watchedDates),
    ...Object.keys(rewatches),
    ...Object.keys(episodes),
    ...Object.keys(activity),
  ]);

  const existingTitles = await client
    .from("title_progress")
    .select("title_id")
    .eq("profile_id", profileId);
  ...
  if (titleIds.size) {
    const rows = [...titleIds].map((titleId) => ({
      profile_id: profileId,
      title_id: titleId,
      status: ignored.has(titleId)
        ? "ignored"
        : watched.has(titleId)
          ? "completed"
          : (episodes[titleId]?.length || 0) > 0
            ? "started"
            : "pending",
      ...
      watched_at: watchedDates[titleId]
        ? new Date(`${watchedDates[titleId]}T12:00:00Z`).toISOString()
        : null,
    ...
    const inserted = await client
      .from("title_progress")
      .upsert(rows, { onConflict: "profile_id,title_id" });
    if (inserted.error) throw inserted.error;
  }
  const staleTitleIds = (existingTitles.data || [])
    .map((row) => row.title_id as string)
    .filter((titleId) => !titleIds.has(titleId));
  if (staleTitleIds.length) {
    const removed = await client
      .from("title_progress")
      .delete()
      .eq("profile_id", profileId)
      .in("title_id", staleTitleIds);
    if (removed.error) throw removed.error;
  }

  const existingEpisodes = await client
    .from("episode_progress")
    .select("title_id,season_number,episode_number")
    .eq("profile_id", profileId);
  if (existingEpisodes.error) throw existingEpisodes.error;
  const localEpisodeKeys = new Set(
    Object.entries(episodes).flatMap(([titleId, values]) =>
      (Array.isArray(values) ? values : []).map((episodeNumber) => `${titleId}:1:${episodeNumber}`),
    ),
  );
  const allEpisodeKeys = new Set([
    ...localEpisodeKeys,
    ...(existingEpisodes.data || []).map(
      (row) => `${row.title_id}:${row.season_number}:${row.episode_number}`,
    ),
  ]);
  const episodeRows = [...allEpisodeKeys].map((key) => {
    ...
    return {
      ...
      watched_at: localEpisodeKeys.has(key)
        ? new Date(activity[titleId] || Date.now()).toISOString()
        : null,
      revision: activity[titleId] || Date.now(),
      device_id: deviceId,
    };
  ...
  if (episodeRows.length) {
    const inserted = await client
      .from("episode_progress")
      .upsert(episodeRows, { onConflict: "profile_id,title_id,season_number,episode_number" });
    if (inserted.error) throw inserted.error;
  }
  ...
  const savedPreferences = await client
    .from("user_preferences")
    .upsert({ profile_id: profileId, preferences }, { onConflict: "profile_id" });
  if (savedPreferences.error) throw savedPreferences.error;
  await syncAchievements(
    profileId,
    storedArray(NEXUS_KEYS.achievements),
    storedRecord(NEXUS_KEYS.achievementRecords),
  );
}
...
export async function syncLocalMarathons(profileId: string) {
  ...
  try {
    local = JSON.parse(localStorage.getItem(NEXUS_KEYS.customMarathons) || "[]");
  } catch {
    return;
  }
  ...
  if (
    remote.error &&
    (remote.error.code === "PGRST204" || /source_local_id/i.test(remote.error.message))
  )
    return;
  if (remote.error) throw remote.error;
  const localIds = new Set(local.map((marathon) => marathon.id));
  const removedRemoteIds = (remote.data || [])
    .filter((entry) => entry.source_local_id && !localIds.has(entry.source_local_id as string))
    .map((entry) => entry.id as string);
  if (removedRemoteIds.length) {
    const removed = await client
      .from("marathons")
      .delete()
      .in("id", removedRemoteIds)
      .eq("owner_profile_id", profileId);
    if (removed.error) throw removed.error;
  }
  for (const marathon of local) {
    const existing = await client
      ...
      .select("id")
      .eq("owner_profile_id", profileId)
      .eq("source_local_id", marathon.id)
      .maybeSingle();
    // Permite que una instalación actual siga funcionando hasta ejecutar 002.
    if (
      existing.error &&
      (existing.error.code === "PGRST204" || /source_local_id/i.test(existing.error.message))
    )
      return;
    if (existing.error) throw existing.error;
    ...
    if (marathon.tasks.length) {
      const rows = marathon.tasks.map((task, index) => ({
        marathon_id: data.id,
        position: (index + 1) * 1000,
        title_id: task.itemId,
        episode: task.episode || null,
...
export async function requestAccountDeletion() {
  ...
  const { error } = await client.functions.invoke("delete-account", {
    body: { confirmation: "DELETE_MY_NEXUS_ACCOUNT" },
  });
  if (error) throw error;
}

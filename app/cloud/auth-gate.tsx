...
async function prepareAccount(session: Session) {
  ...
  await syncProfile(
    session,
    profiles,
    localStorage.getItem(NEXUS_KEYS.activeProfile) || "principal",
    "merge",
  );
}
...
export function AuthGate({ children }: { children: ReactNode }) {
    ...
    client.auth.getSession().then(async ({ data }) => {
      ...
      if (data.session) {
        try {
          ...
          /* CloudWorkspace reintentará en segundo plano */
        }
  ...
  if (screen === "checking")
    return (
      <div className="nexus-auth-loading">
        <span>N</span>
        <p>Preparando Nexus…</p>
      </div>
    );

  return (
    <main className="nexus-auth-gate">
        ...
        {screen === "welcome" ? (
          <>
            <div className="auth-gate-copy">
              <small>TU MULTIVERSO, SIEMPRE CONTIGO</small>
              <h1>Continúa donde lo dejaste.</h1>
              <p>Guarda películas, capítulos, maratones y logros en todos tus dispositivos.</p>
            </div>
            <div className="auth-gate-actions">
              ...
              {!cloudConfigured && (
                <p>La nube no está configurada. Puedes continuar como invitado.</p>
              )}
            </div>
          </>
        ) : (
          <>
            ...
            <form className="auth-gate-form" onSubmit={submit}>
              ...
              {message && (
                <p className="auth-gate-message" role="status">
                  {message}
                </p>
              )}
            </form>
